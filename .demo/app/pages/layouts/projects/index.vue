...
<template>
  <div class="px-4 md:px-6 lg:px-8 pb-20 dark:[--color-input-default-bg:var(--color-muted-950)]">
    <TairoContentWrapper>
      <template #left>
        <TairoInput v-model="filter" icon="lucide:search" rounded="sm" placeholder="Filter projects..." />
      </template>
      <template #right>
        <BaseButton variant="primary" class="w-full sm:w-32" rounded="sm">
        ...
        <div>
          ...
          <div v-else>
            ...
            <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
              <TransitionGroup
                enter-active-class="transform-gpu" enter-from-class="opacity-0 -translate-x-full"
              ...
              >
                <NuxtLink
                  v-for="(item, r) in data?.recent.slice(0, 4)" :key="r" class="group block"
                  ...
                  <BaseCard rounded="sm" elevated-hover class="group-hover:border-primary-500! p-5">
                    <div class="mb-6 flex gap-2">
                      <BaseTooltip :content="item.name">
                        <BaseAvatar :src="item.customer.logo" size="sm" class="bg-muted-100 dark:bg-muted-700" />
                      </BaseTooltip>
                      <div>
                    ...
                    <div class="flex items-center justify-between">
                      <div class="flex items-center gap-2">
                        <BaseTooltip v-for="stack in item.stacks" :key="stack.name" :content="stack.name">
                          <BaseAvatar :src="stack.icon" size="xxs" class="bg-muted-100 dark:bg-muted-700" />
                        </BaseTooltip>
                      </div>
        ...
        <div>
          ...
          <div v-else>
            ...
            <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              ...
              >
                <BaseCard
                  v-for="(item, index) in data?.data" :key="index" rounded="sm" elevated-hover
                  class="hover:border-primary-500!"
                >
                  <NuxtLink class="group" :to="`/layouts/projects/details/${item.slug}`">
                    <div class="p-5">
                    ...
                    >
                      ...
                      <div>
                        <BaseAvatarGroup :avatars="item.team" size="xs" :limit="3" />
                      </div>
                    </div>
                  </NuxtLink>
                </BaseCard>
