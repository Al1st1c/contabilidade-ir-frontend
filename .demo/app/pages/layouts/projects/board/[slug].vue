...
<template>
  <div class="ps-4 md:ps-6 lg:ps-8">
    <div v-if="data?.project !== undefined" class="flex h-12 items-center justify-between pe-4 xl:pe-10">
      ...
      <div class="flex items-center gap-2">
        ...
        <BaseTooltip content="Project details">
          <BaseButton :to="`/layouts/projects/details/${slug}`" size="icon-sm" rounded="full">
    ...
    <div class="flex h-[calc(100vh_-_128px)] grow space-x-6 overflow-auto pt-2">
      <!-- Column -->
      <div v-for="column in board" :key="column.title" class="flex h-full w-72 shrink-0 flex-col">
        <!-- Column header -->
              ...
              <Draggable v-for="task in column.tasks" :key="task.id">
                ...
                >
                  <div class="relative mb-2">
                    ...
                    <BaseHeading as="h4" size="sm" weight="medium" class="line-clamp-2">
                      <span>{{ task.name }}</span>
                    </BaseHeading>
                  </div>
                  <div
