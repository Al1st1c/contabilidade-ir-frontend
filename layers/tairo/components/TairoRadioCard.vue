...
<template>
  <RadioGroupItem :id v-bind="{ ...forward, ...attrs }" as-child>
    <BaseCard :rounded="props.rounded" :variant="props.variant"
      ...
      <slot>
        <Icon v-if="props.icon" :name="props.icon" class="group-data-[state=checked]:text-primary-500 mx-auto size-6" />

        <div>
          <BaseHeading v-if="props.label" as="h4" size="sm" weight="medium" lead="none">
            {{ props.label }}
          </BaseHeading>

          <BaseText v-if="props.subtitle" size="xs" class="text-muted-400">
            {{ props.subtitle }}
          </BaseText>
        </div>
      </slot>

      <RadioGroupIndicator force-mount
        class="data-[state=checked]:text-primary-500 text-muted-300 absolute start-2 top-2 ms-auto">
        <slot name="indicator">
