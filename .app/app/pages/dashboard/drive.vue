...
const storageQuotaMB = computed(() => {
  // 1. Tenta pegar o limite direto da assinatura (sobrescrito ou herdado)
  if (currentSubscription.value?.storageMbLimit)
    return currentSubscription.value.storageMbLimit

  // 2. Tenta pegar o limite do plano associado (caso o anterior esteja ausente)
  if (currentSubscription.value?.plan?.limits?.storage_mb)
    return currentSubscription.value.plan.limits.storage_mb

  // 3. Fallback final (Padrão do plano Gratuito)
  return 250
})
const storageUsedMB = computed(() => {
  // Tenta pegar o uso direto da assinatura (mais preciso)
  if (currentSubscription.value?.storageUsedMb !== undefined)
    return currentSubscription.value.storageUsedMb

  // Fallback para o que vier das estatísticas do drive
  return Number.parseFloat(stats.value.totalSizeMB) || 0
})

const storageRemainingMB = computed(() => {
  return Math.max(0, storageQuotaMB.value - storageUsedMB.value)
})

const storagePercentage = computed(() => {
  if (storageQuotaMB.value === 0)
    return 0
  return Math.min(100, (storageUsedMB.value / storageQuotaMB.value) * 100)
})

const storageStatus = computed(() => {
  const pct = storagePercentage.value
  if (pct >= 100)
    return { level: 'critical', label: 'Esgotado', color: 'danger', icon: 'solar:danger-triangle-bold' }
  if (pct >= 95)
    return { level: 'danger', label: 'Crítico', color: 'danger', icon: 'solar:danger-triangle-bold' }
  if (pct >= 80)
    return { level: 'warning', label: 'Atenção', color: 'warning', icon: 'solar:bell-bold' }
...
async function fetchFiles() {
  ...
  try {
    ...
    if (searchTerms.value)
      params.search = searchTerms.value
...
const filteredResults = computed(() => {
  ...
  if (filters.uploadedBy.client || filters.uploadedBy.system) {
    results = results.filter((item: any) => {
      if (item.type === 'file') {
        if (filters.uploadedBy.client && item.uploadedViaLink)
          return true
        if (filters.uploadedBy.system && !item.uploadedViaLink)
          return true
        return false
      }
      return true
...
// Actions
function openClientFiles(client: any) {
    ...
    <div class="grid grid-cols-12 gap-4">
      ...
      <div class="col-span-12">
        ...
        <div class="grid grid-cols-12 gap-4">
                ...
                <div class="bg-white/10 backdrop-blur-sm rounded-xl p-3 md:p-4 md:min-w-[220px]">
                  ...
                  <div class="flex items-center justify-between mb-2 md:mb-3">
                    <div class="flex items-center gap-2">
                      ...
                      <BaseText size="xs" class="text-white/70 uppercase font-semibold">
                        Armazenamento
                      </BaseText>
                    </div>
                    <!-- Status Badge -->
                  ...
                  <div class="mb-2 md:mb-3">
                    <div class="flex items-baseline gap-1">
                      ...
                      <span class="text-xs md:text-sm text-white/60">/ {{ storageQuotaMB }} MB</span>
                    </div>
                  </div>
          ...
          <AppLimitAlert v-if="(storageStatus.level === 'critical' || storageStatus.level === 'danger') && !loadingSub"
            class="mb-6" title="Espaço de Armazenamento Crítico"
            description="Seu espaço no Drive está quase cheio. Para garantir que seus clientes possam continuar enviando documentos, faça um upgrade no seu plano."
                    ...
                    <BasePlaceholderPage title="Nenhum arquivo encontrado"
                      ...
                      <template #image>
                        <img class="block dark:hidden"
                          src="/img/illustrations/placeholders/flat/accounting-empty-search-v2.png" alt="Sem arquivos">
                    ...
                    <div class="col-span-12 lg:col-span-8 xl:col-span-9">
                      ...
                      <div v-else-if="filteredResults.length === 0" class="py-20">
                        <BasePlaceholderPage title="Nenhum resultado"
                          ...
                          <template #image>
                            <img class="block dark:hidden"
                              src="/img/illustrations/placeholders/flat/accounting-empty-search-v2.png"
                              alt="Sem resultados">
                            <img class="hidden dark:block"
                              src="/img/illustrations/placeholders/flat/accounting-empty-search-v2-dark.png"
                              alt="Sem resultados">
                        ...
                        <BaseCard v-for="result in filteredResults" :key="result.id" rounded="md"
                          ...
                          <div class="flex w-full items-center gap-3 md:gap-4">
                            ...
                            <div v-else
                              ...
                              :class="`bg-${getCategoryInfo(result.category)?.color || 'muted'}-500/10 text-${getCategoryInfo(result.category)?.color || 'muted'}-500`">
                            ...
                            <div class="flex-1 min-w-0">
                              ...
                              <div class="flex flex-wrap items-center gap-1 md:gap-2 mt-0.5">
                                ...
                                <!-- File Info -->
                                <template v-else>
                                  <BaseTag v-if="result.category" rounded="full" size="sm"
                                    :color="getCategoryInfo(result.category)?.color || 'muted'"
                                    class="text-[9px] md:text-[10px] px-1.5 md:px-2">
                                    {{ getCategoryInfo(result.category)?.label || 'Outro' }}
                                  </BaseTag>
                                  <span class="text-[10px] text-muted-400 hidden sm:inline">
                                    {{ formatFileSize(result.fileSize) }}
                                  </span>
                                  <span v-if="result.client"
                                    class="text-[10px] text-muted-400 truncate max-w-[80px] md:max-w-none">
                                    {{ result.client.name }}
                                  </span>
