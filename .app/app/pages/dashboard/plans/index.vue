...
const getPlanPrice = (plan: any) => {
  if (plan.slug === 'free') return 'Grátis'
  ...
  <div class="bg-muted-100 dark:bg-muted-900 min-h-screen">
    <div class="mx-auto flex h-16 w-full max-w-7xl items-center justify-between px-4">
      <NuxtLink to="/dashboard"
        class="text-muted-400 hover:text-primary-500 dark:text-muted-700 dark:hover:text-primary-500 transition-colors duration-300 font-bold flex items-center gap-2">
    ...
    <div class="mx-auto max-w-7xl px-4 pb-20">
      ...
      <div v-if="loading" class="flex flex-col items-center justify-center py-20">
        ...
        <BaseText>Carregando planos e opções...</BaseText>
      </div>

      <div v-else-if="error" class="py-10 text-center">
        <BaseText class="text-error-500 mb-4">{{ error }}</BaseText>
        <BaseButton @click="fetchPlans">Tentar Novamente</BaseButton>
      </div>
                ...
                <TairoRadioCard v-for="plan in filteredPlans" :key="plan.id" rounded="md" :value="plan.slug"
                  ...
                  <div class="flex flex-col items-center p-4">
                    ...
                    <div class="flex items-baseline justify-center gap-1 mb-6">
                      ...
                      <BaseText v-if="plan.slug !== 'free'" size="xs" class="text-muted-500 dark:text-muted-400">/mês
                      </BaseText>
                    </div>
                    ...
                    <ul class="w-full text-left space-y-3 mt-4 border-t border-muted-200 dark:border-muted-800 pt-6">
                      ...
                      <li v-if="plan.limits.storage_mb" class="flex items-center gap-2 text-xs">
                        ...
                        <span class="text-muted-600 dark:text-muted-300" v-if="plan.limits.storage_mb >= 1024">{{
                          Math.round(plan.limits.storage_mb / 1024) }}GB Space</span>
                        <span class="text-muted-600 dark:text-muted-300" v-else>{{ plan.limits.storage_mb }}MB
                          Space</span>
                      </li>
