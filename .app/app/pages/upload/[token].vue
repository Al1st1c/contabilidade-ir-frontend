...
const checklistProgress = computed(() => {
  if (!data.value?.checklist) return 0
      ...
      <div class="col-span-12 lg:col-span-8 space-y-6">
        ...
        <BaseCard class="p-6">
          <div class="mb-6 flex items-center justify-between">
            ...
            <BaseTag color="muted" size="sm">{{ data.checklist.length }} Itens</BaseTag>
          </div>

          <div class="space-y-3">
            <div v-for="item in data.checklist" :key="item.id"
              ...
              <div class="shrink-0 flex items-start pt-1">
                <div class="size-10 rounded-full flex items-center justify-center" :class="[
              ...
              <div class="flex-1 min-w-0">
                <div class="flex items-start justify-between gap-2">
                  <div>
                    <h4 class="font-bold text-muted-800 dark:text-white">{{ item.title }}</h4>
                    <p class="text-sm text-muted-500 mt-0.5">{{ item.description || 'Envie o documento solicitado.' }}
                    </p>
                  </div>
                  <BaseTag v-if="item.isRequired" color="danger" variant="muted" size="sm" class="shrink-0">Obrigatório
                  </BaseTag>
                </div>
                ...
                <div class="mt-4 flex flex-wrap items-center gap-3">
                  <input :id="`input-${item.id}`" type="file" class="hidden"
                  ...
                  <template v-else>
                    ...
                    <button v-if="item.status !== 'approved'" @click="triggerUpload(item.id)"
                      class="text-xs text-muted-400 hover:text-primary-500 underline ml-auto sm:ml-2">
                      Substituir
                    </button>
                  </template>
                </div>
        ...
        <BaseCard class="p-6">
          ...
          <div v-if="data.attachments.filter((a: any) => !a.checklistItemId).length === 0"
            ...
            <p class="text-sm">Nenhum arquivo extra enviado</p>
          </div>

          <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <div v-for="att in data.attachments.filter((a: any) => !a.checklistItemId)" :key="att.id"
              ...
              <div class="min-w-0">
                ...
                <p class="text-xs text-muted-500">{{ (att.fileSize / 1024 / 1024).toFixed(2) }} MB</p>
              </div>
            </div>
          </div>
        </BaseCard>
      ...
      <div class="col-span-12 lg:col-span-4 space-y-6">
        ...
        <BaseCard class="p-6 bg-gradient-to-br from-primary-500/5 to-transparent">
          <div class="flex items-center gap-2 mb-4">
            ...
            <BaseHeading as="h3" size="sm" weight="semibold">Mensagem</BaseHeading>
          </div>
        ...
        <!-- Progress Stats -->
        <BaseCard class="p-6">
          <BaseHeading as="h3" size="sm" weight="semibold" class="mb-6">Resumo</BaseHeading>
        ...
        <BaseCard class="p-6">
          <div class="text-center">
            <p class="text-sm text-muted-500 mb-4">Dúvidas sobre os documentos?</p>
            <a v-if="data.branding.contactPhone"
