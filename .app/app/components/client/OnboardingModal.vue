...
async function skipOnboarding() {
  if (!confirm('Deseja pular a configuração inicial? Você poderá preencher esses dados depois.')) return
      ...
      <DialogContent
        ...
        <div class="p-0 overflow-hidden flex-1 flex flex-col md:flex-row">
          ...
          <div
            ...
            <div v-for="(step, idx) in steps" :key="idx" @click="idx < currentStep ? currentStep = idx : null"
              class="group relative flex items-center gap-3 p-2.5 rounded-xl transition-all duration-300" :class="[
              ...
              <div class="flex flex-col overflow-hidden">
                <BaseText size="xs" weight="bold" class="truncate transition-colors"
          ...
          <div class="flex-1 p-6 md:p-10 overflow-y-auto bg-white dark:bg-muted-900 relative">
            <div class="max-w-md mx-auto h-full flex flex-col">
              ...
              <div v-if="currentStep === 0"
                ...
                <div class="flex items-center justify-center gap-3 py-4">
                  <div
                    ...
                    <BaseText size="xs" weight="medium" class="text-muted-600 dark:text-muted-400">Seguro</BaseText>
                  </div>
                  <div
                    class="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-muted-50 dark:bg-muted-950/50 border border-muted-100 dark:border-muted-800">
                    <Icon name="solar:bolt-bold-duotone" class="size-3.5 text-amber-500" />
                    <BaseText size="xs" weight="medium" class="text-muted-600 dark:text-muted-400">Rápido</BaseText>
                  </div>
                </div>
              </div>
                ...
                <div class="space-y-4">
                  <div class="relative size-32 mx-auto group">
                    <div
                      ...
                      <img v-if="form.photoPreview || client?.photoUrl" :src="form.photoPreview || client.photoUrl"
                  ...
                  <div class="max-w-xs mx-auto">
                    <BaseHeading as="h3" size="lg" weight="bold" class="text-muted-800 dark:text-white">Identificação
                    </BaseHeading>
                    <BaseParagraph size="xs" class="text-muted-500 mt-1 leading-relaxed">Uma foto ajuda nosso time a te
                      reconhecer e personalizar seu atendimento.</BaseParagraph>
                  </div>
                </div>
              </div>
              ...
              <div v-if="currentStep === 2" class="space-y-6 animate-fade-in w-full py-2">
                <div class="grid grid-cols-12 gap-x-4 gap-y-4">
                  <div class="col-span-12 md:col-span-4">
                    <BaseText size="xs" weight="bold" class="text-muted-400 uppercase tracking-wider mb-1.5 block">CEP
                    </BaseText>
                  ...
                  <div class="col-span-12 md:col-span-8">
                    <BaseText size="xs" weight="bold" class="text-muted-400 uppercase tracking-wider mb-1.5 block">
                      Logradouro</BaseText>
                    <BaseInput v-model="form.address" placeholder="Rua, Avenida..." icon="solar:map-point-bold-duotone"
                  ...
                  <div class="col-span-12 md:col-span-3">
                    <BaseText size="xs" weight="bold" class="text-muted-400 uppercase tracking-wider mb-1.5 block">
                      Número</BaseText>
                    <BaseInput v-model="form.addressNumber" placeholder="123" rounded="lg" size="sm"
                  ...
                  <div class="col-span-12 md:col-span-5">
                    <BaseText size="xs" weight="bold" class="text-muted-400 uppercase tracking-wider mb-1.5 block">
                      Bairro</BaseText>
                    <BaseInput v-model="form.neighborhood" placeholder="Seu bairro" rounded="lg" size="sm"
                      class="!bg-muted-50 dark:!bg-muted-950" />
                  </div>

                  <div class="col-span-12 md:col-span-4">
                    <BaseText size="xs" weight="bold" class="text-muted-400 uppercase tracking-wider mb-1.5 block">
                      Complemento</BaseText>
                    <BaseInput v-model="form.addressComplement" placeholder="Apto, Sala..." rounded="lg" size="sm"
                      class="!bg-muted-50 dark:!bg-muted-950" />
                  </div>
                  ...
                  <div class="col-span-12 md:col-span-8">
                    <BaseText size="xs" weight="bold" class="text-muted-400 uppercase tracking-wider mb-1.5 block">
                      Cidade</BaseText>
                    <BaseInput v-model="form.city" placeholder="Ex: São Paulo" icon="solar:city-bold-duotone"
                      rounded="lg" size="sm" class="!bg-muted-50 dark:!bg-muted-950" />
                  </div>

                  <div class="col-span-12 md:col-span-4">
                    <BaseText size="xs" weight="bold" class="text-muted-400 uppercase tracking-widest mb-2 block">UF
                    </BaseText>
                    <BaseInput v-model="form.state" v-maska="'AA'" placeholder="SP" rounded="lg"
                      class="!bg-muted-50 dark:!bg-muted-950" />
                  </div>
                </div>
              </div>
              ...
              <div v-if="currentStep === 3" class="space-y-6 animate-fade-in w-full py-2">
                ...
                <div class="space-y-4">
                  <div>
                    <BaseText size="xs" weight="bold" class="text-muted-400 uppercase tracking-wider mb-1.5 block">Chave
                      PIX (CPF vinculada)</BaseText>
                    <BaseInput v-model="form.pixKey" placeholder="Seu CPF ou Email" icon="fa6-brands:pix" rounded="lg"
                      size="sm" class="!bg-muted-50 dark:!bg-muted-950" />
                  </div>

                  <div class="relative flex py-2 items-center">
                    <div class="flex-grow border-t border-muted-200 dark:border-muted-800"></div>
                  ...
                  <div class="grid grid-cols-12 gap-x-4 gap-y-4">
                    <div class="col-span-12 md:col-span-7">
                      <BaseText size="xs" weight="bold" class="text-muted-400 uppercase tracking-wider mb-1.5 block">
                        Instituição</BaseText>
                      <BaseInput v-model="form.bankName" placeholder="Itaú, Nubank..." icon="solar:bank-bold-duotone"
                        rounded="lg" size="sm" class="!bg-muted-50 dark:!bg-muted-950" />
                    </div>
                    <div class="col-span-12 md:col-span-5">
                      <BaseText size="xs" weight="bold" class="text-muted-400 uppercase tracking-wider mb-1.5 block">
                        Tipo</BaseText>
                      <BaseSelect v-model="form.bankAccountType" rounded="lg" size="sm"
                        class="!bg-muted-50 dark:!bg-muted-950">
                    ...
                    <div class="col-span-6 md:col-span-4">
                      <BaseText size="xs" weight="bold" class="text-muted-400 uppercase tracking-wider mb-1.5 block">
                        Agência</BaseText>
                      <BaseInput v-model="form.bankAgency" placeholder="0001" rounded="lg" size="sm"
                        class="!bg-muted-50 dark:!bg-muted-950" />
                    </div>
                    <div class="col-span-6 md:col-span-8">
                      <BaseText size="xs" weight="bold" class="text-muted-400 uppercase tracking-wider mb-1.5 block">
                        Conta</BaseText>
                      <BaseInput v-model="form.bankAccount" placeholder="00000-0" rounded="lg" size="sm"
                        class="!bg-muted-50 dark:!bg-muted-950" />
                    </div>
                  </div>
                </div>
              ...
              <div v-if="currentStep === 4"
                ...
                <div class="text-center mb-2">
                  ...
                  <BaseHeading as="h3" size="lg" weight="bold" class="text-muted-800 dark:text-white">Conexão GOV.br
                  </BaseHeading>
                ...
                <div class="max-w-xs mx-auto w-full space-y-4">
                  <div>
                    <BaseText size="xs" weight="bold" class="text-muted-400 uppercase tracking-wider mb-1.5 block">Senha
                      GOV.br</BaseText>
                    <BaseInput v-model="form.govPassword" type="password" placeholder="Sua senha segura"
        ...
        <div
          ...
          <BaseButton v-if="currentStep > 0" @click="prevStep" variant="muted" rounded="lg" size="sm"
            class="px-5 h-9 font-bold">
          ...
          <div class="flex items-center gap-3">
            <button v-if="currentStep > 0 && currentStep < steps.length - 1" @click="nextStep"
              class="text-[10px] font-bold text-muted-400 hover:text-muted-600 px-2 uppercase tracking-wider">
              Pular
            </button>
            <BaseButton @click="nextStep" variant="primary" :loading="isLoading" rounded="lg" size="sm"
              class="w-36 h-9 font-bold shadow-lg shadow-primary-500/10">
