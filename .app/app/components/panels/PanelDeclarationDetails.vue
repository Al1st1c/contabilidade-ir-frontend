...
const filteredAttachments = computed(() => {
  ...
  return declaration.value.attachments.filter((a: any) => {
    ...
    // Esconder documentos rejeitados no checklist
    if (a.checklistStatus === 'rejected')
      return false
...
async function handleDelete() {
  ...
  try {
    ...
    if (data && data.success) {
      ...
      emit('saved')
      emit('close')
      // Refaz fetch para atualizar o saldo de IR devolvido na store/cookies
  ...
  <FocusScope
    ...
    <div class="flex-1 overflow-hidden flex flex-col md:flex-row">
      ...
      <div v-if="isLoading" class="flex flex-col items-center justify-center w-full h-full min-h-[400px]">
        <AppPageLoading message="Carregando detalhes do IR..." />
      </div>
          ...
          <div class="p-5 space-y-5">
            ...
            <div class="space-y-3">
              ...
              <div class="space-y-1.5">
                ...
                <div class="flex flex-wrap gap-1.5">
                  <BaseTag v-for="tag in form.tags" :id="`tag-${Math.random()}`" :key="tag" size="sm" variant="none"
                    class="group relative text-[10px] pr-5 bg-primary-500 text-white">
                    {{ tag }}
                    <button
                      class="absolute right-1 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 hover:text-danger-500 transition-all"
